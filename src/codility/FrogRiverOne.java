...
class FrogRiverOne {
    public int solution(int X, int[] A) {
        // Base case if X greater than array of leaves, no solution exist.
        if (X > A.length)
            return -1;

        // Create a leaves array to monitor already places leaves at position.
        int[] leaves = new int[X];
        int count = 0;
        for (int i = 0; i < A.length; i++) {
            // If current element is less than or equal to X then only increase
            // the index in the leaves array and also increment count.
            if (A[i] <= X) {
                if (leaves[A[i] - 1] == 0) {
                    leaves[A[i] - 1]++;
                    count++;
            ...
            // If count equal to X then leaves array is filled.
            if (count == X)
                return i;
        }
        return -1;
