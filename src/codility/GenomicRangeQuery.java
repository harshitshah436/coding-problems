 ...
 * arrays P and Q, each consisting of M integers. The K-th query (0 leq K < M)
 ...
 * 
 * For example, consider string S = CAGCCTA and arrays P, Q such that: P[0] = 2
 * Q[0] = 4 P[1] = 5 Q[1] = 5 P[2] = 0 Q[2] = 6 The answers to these M = 3
 ...
 * language).
 * 
 * For example, given the string S = CAGCCTA and arrays P, Q such that: P[0] = 2
 * Q[0] = 4 P[1] = 5 Q[1] = 5 P[2] = 0 Q[2] = 6 the function should return the
 * values [2, 4, 1], as explained above.
 * 
 * Assume that:
 * 
 ...
 * range [0..N - 1]; -P[K] leq Q[K], where 0 leq K < M; -string S consists only
...
public class GenomicRangeQuery {
    public int[] solution(String S, int[] P, int[] Q) {
        ...
        // Array to store 'acg' with prefix sums. No need to store 't'.
        int[][] acg = new int[3][S.length() + 1];
        ...
        for (int i = 0; i < S.length(); i++) {
            ...
            // Calculate prefix sums for current index.
            acg[0][i + 1] = acg[0][i] + a;
            acg[1][i + 1] = acg[1][i] + c;
            acg[2][i + 1] = acg[2][i] + g;
        }
        ...
        for (int i = 0; i < P.length; i++) {
            ...
            // we need to add 1 to Q[i], because our acg[0][0], acg[0][1] and
            // acg[0][2] have 0 values by default, look above acg[0][i+1] =
            // acg[0][i] + a.
            int to = Q[i] + 1;

            if (acg[0][to] - acg[0][from] > 0)
                result[i] = 1;
            else if (acg[1][to] - acg[1][from] > 0)
                result[i] = 2;
            else if (acg[2][to] - acg[2][from] > 0)
                result[i] = 3;
            else
                result[i] = 4;
        }
