...
/**
 * 
 * Hard problem to understand.
 * 
 * Task URL: https://codility.com/programmers/task/stone_wall/
 * 
 ...
 * height of the wall's left end and H[N-1] is the height of the wall's right
 * end.
 * 
 ...
 * 
 * For example, given array H containing N = 9 integers: H[0] = 8 H[1] = 8 H[2]
 * = 5 H[3] = 7 H[4] = 9 H[5] = 8 H[6] = 7 H[7] = 4 H[8] = 8 the function should
 * return 7. The figure shows one possible arrangement of seven blocks.
 * 
 * 
 * 
 * Assume that:
...
public class StoneWall {
    public int solution(int[] H) {
        int blocks = 0;
        Stack<Integer> blocks_stack = new Stack<Integer>();

        for (int i = 0; i < H.length; ++i) {
            while (!blocks_stack.empty() && H[i] < blocks_stack.peek()) {
                blocks_stack.pop();
            }

            if (blocks_stack.empty() || H[i] > blocks_stack.peek()) {
