 ...
 * has the form "VW" where V and W are properly nested strings.
 * 
...
public class Nesting {
    public int solution(String S) {
        Stack<Character> sNest = new Stack<Character>();
        for (int i = 0; i < S.length(); i++) {
            char ch = S.charAt(i);
            if (ch == '(') {
                sNest.push(ch);
            } else if (ch == ')') {
                // remember to check this condition for "())" type of strings.
                if (!sNest.empty())
                    sNest.pop();
                else
                    sNest.push(ch);
