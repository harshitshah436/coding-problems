 ...
 * 30 = 100
 * 
 * Assume that: X, Y and D are integers within the range [1..1,000,000,000]; X
 * \leq Y. Complexity:
 * 
...
class FrogJmp {
    public int solution(int X, int Y, int D) {
        // Base case if X is already greater than Y, no need of extra steps
        if (X >= Y) {
            return 0;
        }
        // Check if the distance is divisible or not
        if ((Y - X) % D == 0) {
            return (Y - X) / D;
        } else {
            return (Y - X) / D + 1;
