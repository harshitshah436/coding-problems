...
public class StrSymmetryPoint {
    public int solution(String S) {
        ...
        // For empty string with length 0 and all other even length strings it
        // should return -1.
        if (S.length() % 2 == 0)
            return -1;
        if (S.length() == 1)
        ...
        int i = 0;
        // loop through whole string and return index when charachters from both
        // end differs.
        for (i = 0; i <= S.length() / 2; i++) {
            int curI = i, endI = S.length() - i - 1;
            if (S.charAt(curI) != S.charAt(endI))
                return -1;
        }
        return --i == S.length() / 2 ? i : -1;
