 ...
 * 
 * For example, given: A[0] = 3 A[1] = 1 A[2] = 2 A[3] = 4 A[4] = 3 the function
...
public class TapeEquilibrium {
    public int solution(int[] A) {
        ...
        // Remember to change this index from 1 to N-1 as given example in the
        // question.
        for (int i = 1; i < A.length; i++) {
            sum_left += A[i - 1];
            sum_right = total_sum - sum_left;
            minDistance = Math.min(minDistance, Math.abs(sum_left - sum_right));
        }
        return minDistance;
    }
